...
// fetchCostData fetches cost info for all dependencies from the Go module proxy.
func fetchCostData(client *registry.Client, deps []types.Dependency) ([]types.CostInfo, error) {
	goProxy := registry.NewGoProxyClient(client)
	...
	for i, dep := range deps {
		g.Go(func() error {
			...
			info, err := goProxy.FetchPackageInfo(dep.Name, version)
			if err != nil {
